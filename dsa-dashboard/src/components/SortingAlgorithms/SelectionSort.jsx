...
export default function SelectionSort() {
  ...
  const selectionSort = async (arr) => {
    for (let i = 0; i < arr.length - 1; i++) {
      ...
      for (let j = i + 1; j < arr.length; j++) {
        ...
        setComparison(`Comparing ${arr[j]} with current minimum ${arr[minIndex]}`);
        setAnimationStep(prev => prev + 1);
        await new Promise((r) => setTimeout(r, 1000));

        if (arr[j] < arr[minIndex]) {
          minIndex = j;
          setComparison(`New minimum found: ${arr[minIndex]}`);
          setAnimationStep(prev => prev + 1);
          await new Promise((r) => setTimeout(r, 1000));
        }
      }
      ...
      if (minIndex !== i) {
        setComparison(`Swapping ${arr[i]} with ${arr[minIndex]}`);
        setAnimationStep(prev => prev + 1);
        await new Promise((r) => setTimeout(r, 1000));
  ...
  const handleSort = async () => {
    ...
    setHighlightIndices([]);
    setAnimationStep(0);
    setComparison('Starting Selection Sort...');
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="mb-8">
        ...
        {isDefinitionOpen && (
          <div className="mt-4 p-4 bg-white rounded-lg shadow-md">
            <h3 className="text-lg font-semibold text-pink-700 mb-2">What is Selection Sort?</h3>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-semibold mb-4 text-pink-600">Selection Sort</h2>
          {error && (
