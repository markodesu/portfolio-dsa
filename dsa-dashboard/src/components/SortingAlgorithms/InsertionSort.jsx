...
export default function InsertionSort() {
  ...
  const insertionSort = async (arr) => {
    for (let i = 1; i < arr.length; i++) {
      ...
      setComparison(`Comparing ${key} with ${arr[j]}`);
      ...
      while (j >= 0 && arr[j] > key) {
        ...
        j--;
        setHighlightIndices([i, j]);
        setComparison(`Moving ${arr[j + 1]} to the right`);
        setAnimationStep(prev => prev + 1);
        await new Promise((r) => setTimeout(r, 1000));
      }
  ...
  const handleSort = async () => {
    ...
    setHighlightIndices([]);
    setAnimationStep(0);
    setComparison('Starting Insertion Sort...');
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="mb-8">
        ...
        {isDefinitionOpen && (
          <div className="mt-4 p-4 bg-white rounded-lg shadow-md">
            <h3 className="text-lg font-semibold text-pink-700 mb-2">What is Insertion Sort?</h3>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-semibold mb-4 text-pink-600">Insertion Sort</h2>
          {error && (
