...
export default function QuickSort() {
  ...
  const partition = async (arr, low, high) => {
    ...
    let i = low - 1;
    setPivotIndex(high);
    setComparison(`Pivot: ${pivot}`);

    for (let j = low; j < high; j++) {
      setHighlightIndices([i, j, high]);
      ...
      if (arr[j] < pivot) {
        ...
        setComparison(`${arr[j]} ≥ ${pivot}, no swap needed`);
        await new Promise((r) => setTimeout(r, 1000));
      }
    }
  ...
  const handleSort = async () => {
    ...
    setPivotIndex(null);
    setAnimationStep(0);
    setComparison('Starting Quick Sort...');
        ...
        {isDefinitionOpen && (
          <div className="mt-4 p-4 bg-white rounded-lg shadow-md">
            ...
            <ol className="list-decimal list-inside text-gray-700 space-y-2">
              ...
              <li>Recursively apply the same steps to the sub-arrays</li>
